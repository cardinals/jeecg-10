...
@Service("wuyeYzglYzjtcyService")
@Transactional
public class WuyeYzglYzjtcyServiceImpl extends CommonServiceImpl implements WuyeYzglYzjtcyServiceI {
	 ...
	 * @return
	 */
	private void doAddBus(WuyeYzglYzjtcyEntity t) throws Exception{
		//-----------------sql增强 start----------------------------
	 	//-----------------sql增强 end------------------------------
	 ...
	 * @return
	 */
	private void doUpdateBus(WuyeYzglYzjtcyEntity t) throws Exception{
		//-----------------sql增强 start----------------------------
	 	//-----------------sql增强 end------------------------------
	 ...
	 * @return
	 */
	private void doDelBus(WuyeYzglYzjtcyEntity t) throws Exception{
	    //-----------------sql增强 start----------------------------
	 	//-----------------sql增强 end------------------------------
 	...
 	private Map<String,Object> populationMap(WuyeYzglYzjtcyEntity t){
		...
		map.put("jtcyxm", t.getJtcyxm());
		map.put("hzgx", t.getHzgx());
		map.put("xb", t.getXb());
		map.put("sfzh", t.getSfzh());
		map.put("gzxz", t.getGzxz());
		map.put("gzdw", t.getGzdw());
		map.put("bz", t.getBz());
		return map;
	}
	 ...
	 * @return
	 */
 	public String replaceVal(String sql,WuyeYzglYzjtcyEntity t){
 		...
 		sql  = sql.replace("#{jtcyxm}",String.valueOf(t.getJtcyxm()));
 		sql  = sql.replace("#{hzgx}",String.valueOf(t.getHzgx()));
 		sql  = sql.replace("#{xb}",String.valueOf(t.getXb()));
 		sql  = sql.replace("#{sfzh}",String.valueOf(t.getSfzh()));
 		sql  = sql.replace("#{gzxz}",String.valueOf(t.getGzxz()));
 		sql  = sql.replace("#{gzdw}",String.valueOf(t.getGzdw()));
 		...
 		if(StringUtil.isNotEmpty(cgJavaValue)){
			...
			try {
				...
				if(obj instanceof CgformEnhanceJavaInter){
					...
					javaInter.execute("wuye_yzgl_yzjtcy",data);
				}
			} catch (Exception e) {
				e.printStackTrace();
