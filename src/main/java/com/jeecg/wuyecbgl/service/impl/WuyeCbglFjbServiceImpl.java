...
@Service("wuyeCbglFjbService")
@Transactional
public class WuyeCbglFjbServiceImpl extends CommonServiceImpl implements WuyeCbglFjbServiceI {
	 ...
	 * @return
	 */
	private void doAddBus(WuyeCbglFjbEntity t) throws Exception{
		//-----------------sql增强 start----------------------------
	 	//-----------------sql增强 end------------------------------
	 ...
	 * @return
	 */
	private void doUpdateBus(WuyeCbglFjbEntity t) throws Exception{
		//-----------------sql增强 start----------------------------
	 	//-----------------sql增强 end------------------------------
	 ...
	 * @return
	 */
	private void doDelBus(WuyeCbglFjbEntity t) throws Exception{
	    //-----------------sql增强 start----------------------------
	 	//-----------------sql增强 end------------------------------
 	...
 	private Map<String,Object> populationMap(WuyeCbglFjbEntity t){
	 ...
	 * @return
	 */
 	public String replaceVal(String sql,WuyeCbglFjbEntity t){
 		...
 		sql  = sql.replace("#{gtbid}",String.valueOf(t.getGtbid()));
 		...
 		sql  = sql.replace("#{yblx}",String.valueOf(t.getYblx()));
 		sql  = sql.replace("#{bl}",String.valueOf(t.getBl()));
 		sql  = sql.replace("#{csds}",String.valueOf(t.getCsds()));
 		...
 		if(StringUtil.isNotEmpty(cgJavaValue)){
			...
			try {
				...
				if(obj instanceof CgformEnhanceJavaInter){
					...
					javaInter.execute("wuye_cbgl_fjb",data);
				}
			} catch (Exception e) {
				e.printStackTrace();
