...
<body>
	<div class="easyui-layout" fit="true" scroll="no">
		<div  data-options="region:'west',title:'房间收费项目设置',split:true" style="width:200px;overflow: auto;">
	        <div id="orgTree" class="ztree"></div>
		</div>
		<div data-options="region:'center',border:false" style="text-align: center;">
			 	<iframe id="listFrameSflist" src=""  frameborder="no" width="100%" height="100%"></iframe>
		</div>
	</div>
</body>
</html>
...
function zTreeOnLeftClick(event, treeId, treeNode) {
	...
	var url = "wuyeSfglFjsfxmszController.do?list2&id=" + curSelectNode.id+"&parentId="+curSelectNode.parentId;
/* 	if(curSelectNode.parentId=="0"){
 	...
 	for(var i=0;i<zNodes.length;i++){
		if(parentId==zNodes[i].parentId){			
			$("#listFrameSflist").attr("src","");
			return false;
		}
	} 
	$("#listFrameSflist").attr("src", url);
};
/**
 * 树右击事件
 */
	...
	jQuery.ajax({  
        ...
        success:function(data){ //请求成功后处理函数。
		    if(data.success){
				...
				$("#listFrameSflist").attr("src", "about:blank");
		    }else{
		    	alert(data.msg);
		    }
        }  
