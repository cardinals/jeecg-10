...
<div class="easyui-layout" fit="true"
	...
	<div region="west" split="true" title="" style="width:500px;">
		<t:datagrid name="wuyeSfglDkxxzbList" checkbox="true"
			pagination="true" fitColumns="false" title="银行代扣信息主表"
			actionUrl="wuyeSfglDkxxzbController.do?datagrid" idField="id"
			...
			<t:dgCol title="银行编码" field="yhbm" dictionary="dkrDkyh" queryMode="single" width="90"></t:dgCol>
			...
			<t:dgToolBar title="录入" icon="icon-add"
				url="wuyeSfglDkxxzbController.do?goAdd" funname="add"></t:dgToolBar>
			<t:dgToolBar title="编辑" icon="icon-edit"
				url="wuyeSfglDkxxzbController.do?goUpdate" funname="update"></t:dgToolBar>
			<t:dgToolBar title="删除"  icon="icon-remove" url="wuyeSfglDkxxzbController.do?doBatchDel" funname="deleteALLSelect"></t:dgToolBar>
			<t:dgToolBar title="详情" icon="icon-search"
				url="wuyeSfglDkxxzbController.do?goUpdate" funname="detail"></t:dgToolBar>
...
<script type="text/javascript">
	...
	function doCreate(id){
		var url = "wuyeSfglYhdkxxController.do?list&zbid="+id;
		
		parent.layer.confirm('确定生成当月扣款?', {icon: 3, title:'提示'}, function(index){
			//异步执行参数发送，以及添加操作
			 $.post("wuyeSfglDkxxzbController.do?insertDaikou",{dkid:id},function(data){
