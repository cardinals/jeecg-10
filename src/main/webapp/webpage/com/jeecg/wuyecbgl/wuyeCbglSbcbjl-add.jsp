 ...
 <body>
   	<div class="easyui-layout" fit="true" scroll="no">
		...
		<div data-options="region:'center',border:false" style="text-align: center;">
			 <t:formvalid formid="formobj" dialog="true" usePlugin="password" layout="table" action="wuyeCbglSbcbjlController.do?doAdd" >
		...
		<table style="width: 600px; display: none;" cellpadding="0" cellspacing="1" class="formtable" id="tab">
				...
				<tr>
					...
					<td class="value">
							  ...
							  <input id="ybsx" name="ybsx" type="text" disabled="disabled" class="inputxt" ignore="ignore" value="房间表" readonly="readonly"/> 
							<span class="Validform_checktip"></span>
...
<!-- 					     	<input id="ybid" name="ybid" type="text" class="form-control" ignore="ignore" popup="false" onclick="" />		 -->	 		     	
				...
				<tr>
					...
					<td class="value">
					     	 <input id="byzs" name="byzs" type="text" style="width: 150px" datatype="/^(-?\d+)(\.\d+)?$/" class="inputxt"  ignore="ignore" value="0.0"/>
							<span class="Validform_checktip"></span>
				...
				<tr>
					...
					<td class="value">
					     	 <input id="hbyl" name="hbyl" type="text" style="width: 150px" datatype="/^(-?\d+)(\.\d+)?$/" class="inputxt" ignore="ignore" value="0.0" />
							<span class="Validform_checktip"></span>
...
/*  	  var fjb = "popupClick(this,'id,ybbh,bl','ybid,ybbh,bl','cbgl_fjb_sb')";
 	  var gtb = "popupClick(this,'id,ybbh,bl','ybid,ybbh,bl','cbgl_gtb_sb')";
		...
		$("select#ybbh").change(function(){
	 		 if($('#ybbh option:selected').text()!="-- 请选择 -- "){				
	 			 $.ajax({
	 		        type: 'POST',  
	 		        dataType : "json",  
	 		        url: 'wuyeCbglSbcbjlController.do?getYbxx',
		...
		$("#byzs").change(function(){
			$("#byyl").val("");
 			if(parseFloat($("#byzs").val()) < parseFloat($("#syzs").val())){
				alert("请填写正确的指数！")
				$("#byyl").val("");
				return false;
			}
...
function loadTree() {	
	jQuery.ajax({  
        ...
        type: 'POST',  
        dataType : "json",  
        url: 'wuyeCbglSbcbjlController.do?getTreeData', //请求的action路径  
	...
	var ztreeCreator = new ZtreeCreator('orgTree',"wuyeCbglSbcbjlController.do?getTreeData",zNodes)
...
function zTreeOnLeftClick(event, treeId, treeNode) {
	...
	if(treeNode.glqlx != '1' && treeNode.glqlx != '2' && treeNode.glqlx != '3' && treeNode.glqlx != '4'){
		...
		$.ajax({
	        type: 'POST',  
	        dataType : "json",  
	        url: 'wuyeCbglSbcbjlController.do?getYbbh',
	        data:'fjid='+parentId,
	        success:function(data){	        	
